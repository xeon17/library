...
const {data: authors} = await useFetch(apiUrl + "/authors/", {
  ...
  transform: (authors) => {
    if (authors.documents && Array.isArray(authors.documents)) {
      return authors.documents.map((document) => ({
        id: document.id,
        name: document.name,
      }));
    }
    ...
    <div class="rounded-lg border bg-white p-4 sm:p-7">
      <form @submit.prevent="submitForm">
        ...
        >
          ...
          <div class="sm:col-span-9">
            <USelectMenu v-model="selectedAuthor" value-attribute="id" option-attribute="name" :options="authors" multiple/>
          </div>
