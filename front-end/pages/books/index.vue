          ...
          <div class="flex flex-col gap-y-6 py-3 px-4">
            <div class="flex justify-between items-center">
              <div class="flex flex-col gap-y-2">
                ...
                <p class="text-sm text-gray-500">A list of all the books in the library.</p>
              </div>
            ...
            <div class="relative max-w-xs">
              ...
              <input type="text" v-model="search" @change="filterBooks" name="hs-table-search" id="hs-table-search"
          ...
          <div class="overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500">Category</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500">Author</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500">Genre</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500">Publisher</th>
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500">Action</th>
              </tr>
              </thead>
              <tbody class="divide-y divide-gray-200">
              <tr class="hover:bg-gray-50 hover:cursor-pointer" v-for="book in books" :key="book.id">
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                  <div>{{ book.title }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ book.category.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ book.author.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ book.genre.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ book.publisher.name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex gap-x-3 items-center">
                    ...
                    >
                      <Icon class="mr-1 h-5 w-5" name="heroicons:information-circle"/>
                      Details
                    </button>
                    <div v-if="role === 'Librarian' || role === 'Admin'" class="flex gap-x-3 items-center">
                      <button
                          class="flex items-center rounded border border-blue-500 px-2 py-2 font-medium text-blue-500 hover:bg-blue-500 hover:text-white"
                          type="button"
