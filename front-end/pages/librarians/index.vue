        ...
        <div class="border bg-white rounded-lg divide-y divide-gray-200">
          <div class="flex flex-col gap-y-6 py-3 px-4">
            <div class="flex justify-between items-center">
              <div class="flex flex-col gap-y-2">
                ...
                <p class="text-sm text-gray-500">
                  Unlocking the World of Knowledge: Your Essential Resource for Librarians and Library Enthusiasts.</p>
              </div>
              <div v-if="role === 'Admin'">
                <button
                ...
                >
                  <Icon class="mr-1 h-5 w-5" name="material-symbols:person-book"/>
                  Add librarian
                </button>
              </div>
            </div>
            <div class="relative max-w-xs">
              ...
              <input type="text" v-model="search" @change="filterLibrarians" name="hs-table-search" id="hs-table-search"
          ...
          <div class="overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr class="hover:bg-gray-50 hover:cursor-pointer" v-for="librarian in librarians" :key="librarian.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ librarian.email }}</td>
                <td v-if="librarian.birthDate === null" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">Unknown</td>
                <td v-else class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ formatDate(librarian.birthDate) }}</td>
                <td v-if="librarian.lastLogin === null" class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">Did
                  not login yet
                </td>
